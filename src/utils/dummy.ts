...
const CLOUD_FRONT_BASE = 'https://d1ie3vue30kkmc.cloudfront.net'
 ...
 * @length 13
 */
export const dummyVideoData = Object.freeze(
  [
    {
      description:
  ...
  ].map(({ splatFileName, ...video }, index) => ({
    ...video,
    id: `${index}`,
    splatFileSrc:
      splatFileName !== undefined
        ? `${CLOUD_FRONT_BASE}/${splatFileName}.splat`
        : SPLAT_FALLBACK,
  }))
)

export const dummyVideoDataMap = Object.freeze(
  Object.fromEntries(dummyVideoData.map(video => [video.id, video]))
)
